...
Os repositórios de orçamento e pagamento usam PostgreSQL por padrão, com migrations Flyway e seed limpo em `src/main/resources/db/migration/`. O modo em memória fica restrito ao profile de testes para validar domínio, controllers e transições de estado sem depender de banco externo. A publicação real em SNS/SQS permanece no backlog de mensageria.
...
O runtime usa adapters PostgreSQL para `orcamento`, `orcamento_item` e `pagamento`. O profile `postgresql` executa as migrations Flyway para o database `oficina_billing`.
